...
스키마·시드에 직접 영향을 주는 4건입니다. **이게 정해지지 않은 채 시작하면 스키마를 다시 짜게 됩니다.**

| ID | 문제 | 권장 |
|---|---|---|
| [D-07](./05-decisions.md) | 카테고리에서 '주방'이 삭제됐는데 카드에는 남아있음 | `living`에 흡수 + 301 |
| [D-08](./05-decisions.md) | **필터 칩이 여행·숙소·맛집 카드와 매칭되지 않음** — 칩을 누르면 새 카드가 전부 사라짐 | 태그 2계층 분리 |
| [D-09](./05-decisions.md) | `재구매/재방문/재숙박 의사`가 3가지 표현으로 분산 | `wouldRepeat` 단일 통합 |
| [D-12](./05-decisions.md) | `COMPARISON` 타입 MVP 포함 여부 | v1.1 연기 (4주 유지) |

나머지 10건과 근거는 → [`05-decisions.md`](./05-decisions.md)
...
1. **위 4건(D-07~D-12) 결정** — [`05-decisions.md`](./05-decisions.md)의 `**결정**:` 칸을 채우기
2. `create-payload-app` 스캐폴딩 + 컬렉션 스키마 정의 ([W1 체크리스트](./04-progress.md))
